...
const DateLabel = ({from, till, content}: { from: string, till?: string, content?: string }) => (
    <div className="text-opacity-90 flex flex-col text-zinc-600 text-sm">
        {content ? <span>{content}</span> : <span>{from} - {till ?? 'PRESENT'}</span>}
        <span>{getYearsSince(from, till)} Yrs+</span>
    </div>
);

const ExperienceSection = ({children, asItem = true}: { children: React.ReactNode, asItem?: boolean }) => (
...
const TechHighlight = ({children}: { children: React.ReactNode }) => (
...
export const experiences: Experience[] = [
    {
        ...
        date: <DateLabel from="2024"/>,
        body: (
            <div className="flex flex-col gap-2">
                <section>
                    Working as a <TechHighlight>Junior Full-Stack Developer</TechHighlight>.
                </section>
                {[
                    <>
                        Currently developing a <TechHighlight>React with Typescript, Redux (Toolkit) &
                        SCSS</TechHighlight> web app
                        <br/>
                        for client-facing layouts based on server-defined configurations.
                    </>,
                    <>
                        Developed full-stack features with <TechHighlight>React & NestJS with Typescript,
                        MongoDB</TechHighlight>
                        <br/>
                        for a client project in the trading domain.
                    </>,
                    <>
                        Developed automation testing framework with <TechHighlight>C# .NET & Playwright</TechHighlight>
                        <br/>
                        for integration into CI pipelines across microservices repositories.
                    </>,
                ].map((content, i) => (
                    <ExperienceSection key={i}>{content}</ExperienceSection>
                ))}
            </div>
        ),
        techStack: "React, Redux, Scss, NestJS, MongoDB, C# .NET, Playwright, Docker ...",
    ...
    },
    {
        ...
        date: <DateLabel from="2022" till="2024" content="January 2022 - June 2024"/>,
        body: (
            <div className="flex flex-col gap-2">
                <ExperienceSection asItem={false}>
                    Worked as a <TechHighlight>Technical Support Engineer</TechHighlight> & contributed as
                    a <TechHighlight>Full-stack Engineer</TechHighlight> in the support department.
                </ExperienceSection>
            </div>
        ),
        techStack:
    ...
    },
    {
        ...
        body: (
            <div className="flex flex-col gap-2">
                <ExperienceSection asItem={false}>
                    Developed a dynamic sports court rental app as part of my final graduation project.
                </ExperienceSection>
                <ExperienceSection asItem={false}>
                    Users could rent and also publish a court which can be rented by others.
                </ExperienceSection>
                <ExperienceSection asItem={false}>
                    <TechHighlight>Utilized Expo framework for real-time notifications</TechHighlight>
                </ExperienceSection>
            </div>
        ),
        techStack:
